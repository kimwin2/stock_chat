...
**링크가 만료돼도 크롤은 계속 돈다** (2026-08-11 부터). 계정이 이미 가입돼
있으면 캐시된 채널 id 나 저장된 최신 메시지 지문으로 채널을 다시 찾는다
(`crawl._find_joined_channel`). 로그에 `[복구]` 가 보이면 이 경로를 탄 것 —
크롤은 정상이지만 config.yaml 링크는 갱신해 두는 게 좋다 (캐시가 날아가고
raw 도 없는 최악의 경우 대비). 또한 crawl 이 실패해도 번들이 만들어졌으면
배포는 진행된다 (run.py exit 0) — 실패는 Actions 경고 주석으로 표시된다.
