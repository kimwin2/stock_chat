...
| 워크플로 | 주기 | 하는 일 |
|---|---|---|
| `hourly.yml` | KST 05~24시, 30분마다 시도 | 최근 2일 크롤 → **오늘 요약만 다시 생성** → 배포. 약 1~2분 |
...
GitHub cron 은 정확한 시계가 아니다 — 부하에 따라 밀리고, 예약된 실행의 절반 이상을
그냥 버리기도 한다 (2026-08 실측). 그래서 30분 간격으로 두 번씩 시도해
시간당 최소 1회 실행을 노린다. 그래도 "정확히 매시"가 아니라 "대략"으로 보면 된다.
